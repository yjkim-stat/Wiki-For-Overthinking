...
| `data/abstracts/` | pipeline only | One row per *announced* paper, per category-day: what was published, as against `data/papers/` which is what the group tracks. Committed — re-fetching costs one request per paper. |
