...
- **A deployment that carries this repository as a template will accumulate
  deltas against it, and needs a register of its own.** Keep what can live in
  `pipelines/local/` there, where a wholesale replacement of a general file
  leaves it alone. Anything that cannot — a schema change, or a fix inside a
  general-purpose function — has to be written down somewhere that gets read
  *before* a file is replaced, and the suite run after. Three collection fixes
  were lost exactly that way once: they lived in general-purpose files, were
  registered nowhere, and their removal broke no test because each one only
  makes collection quieter.
...
| `scripts/`, `tests/` | yes | `daily.sh`, `new_topic.sh`; and the suite, which touches neither the network nor the real `data/`. `tests/test_local*.py` and `test_model_kind.py` sit apart from the general-purpose test files on purpose — a wholesale replacement of one of those would revert what they guard without failing anything. |
| `docs/commit/` | **required** | One note per commit, staged with the commit it explains. See the rule above. |
| `docs/issues/` | yes | A defect or design question investigated but not yet acted on. Move it to `docs/solved/` once it is. |
