...
| `decks/` | yes, in the YAML | Hand-written presentations, one directory each: `deck.yaml`, one file per slide under `slides/`, pictures under `assets/`, and `build/` holding the generated HTML. **The YAML is the source; `build/` is regenerated wholesale and never committed**, as are the figure crops under `assets/figures/` — see the deck's own `assets/FIGURES.md` for why. Build with `python3 decks/build.py <deck>`; the vocabulary a slide is written in is in [`decks/README.md`](decks/README.md). Unlike `outputs/`, nothing in the pipeline writes here — a deck is authored, not derived from `data/`. |
