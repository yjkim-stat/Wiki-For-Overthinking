...
- **Methods**: AdaMix (dual-adapter decoupled optimization + difficulty-aware routing), LoRA, [GRPO](../../../../wiki/methods/grpo.md), task arithmetic (adapter interpolation), DPO (baseline), ShorterBetter (baseline), TLMRE (baseline), ModelMerging (baseline), CoT-Valve (baseline), SwitchCoT (baseline)
