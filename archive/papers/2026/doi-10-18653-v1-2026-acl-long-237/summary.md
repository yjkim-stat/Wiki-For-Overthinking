...
- **Methods**: MINER (uncertainty-driven intrinsic reward + token-level focal credit assignment + adaptive advantage calibration), GRPO (baseline), DAPO (baseline), REINFORCE++ (baseline), GSPO (baseline), UCAS (baseline), BAPO, KL-Cov, Clip-Cov, Entropy-Adv (exploration-enhancement baselines)
