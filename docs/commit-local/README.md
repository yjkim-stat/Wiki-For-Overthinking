...
| 0044 | [Three cron jobs that never started](0044-three-cron-jobs-that-never-started.md) | fix · operational | The crontab redirects into directories that did not exist, so the shell failed before the script ran and each script's own `mkdir -p` never executed |
| 0045 | [The job with nothing to run against](0045-the-job-with-nothing-to-run-against.md) | chore · operational | Its deployment is not on this host, so the 03:00 job is commented out rather than repaired or deleted |
| 0046 | [Fifteen keywords that buy three papers](0046-fifteen-keywords-that-buy-three-papers.md) | config | Intervention vocabulary added after measuring that it moves the 30-day count from 62 to 65; the band this literature sits in is reached by `inbox/`, not by keywords |
