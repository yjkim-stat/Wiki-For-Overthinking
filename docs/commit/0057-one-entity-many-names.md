...
| **Scope** | `pipelines/local/aliases.py`, `config/concept-aliases.yaml`, `pipelines/common/config.py`, `pipelines/enrich/concepts.py`, `tests/test_local_aliases.py`, `docs/LOCAL-DELTAS.md` |
| **Kind** | feat |
...
is why the delta is two lines in `enrich/concepts.py` rather than one, and why
`docs/LOCAL-DELTAS.md` says restoring only the obvious one is worse than
restoring neither.
