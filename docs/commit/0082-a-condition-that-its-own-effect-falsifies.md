# 0082 — A condition that its own effect falsifies
...
the whole thing is auditable after the fact — the same argument that keeps a
one-off migration script in the tree after it has been run.
