# 0084 — A counter that counts attempts

| | |
| --- | --- |
| **Commit** | `fix(render): definitions_queued counts tasks filed, not attempts` |
| **Scope** | `pipelines/render.py`, `tests/test_local.py` |
| **Kind** | fix |
...
on either side rather than trusting a return value, and
...
registered in alongside the reserve it sits next to.
