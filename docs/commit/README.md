...
| 0058 | [A model is not a dataset](0058-a-model-is-not-a-dataset.md) | feature · breaking | A checkpoint and a corpus answer different questions; a schema change has no seam to hide behind |
| 0059 | [Local extensions in a package of their own](0059-local-extensions-in-a-package-of-their-own.md) | feature | A placeholder is a false entity, a record with no abstract cannot be scored, and one cap starved the wiki |
| 0060 | [Three fixes that only made collection quieter](0060-three-fixes-that-only-made-collection-quieter.md) | fix | Under-collecting looks exactly like a quiet day, and a test inside a replaceable file cannot guard it |
| 0061 | [The archive this repository keeps](0061-the-archive-this-repository-keeps.md) | feature · breaking | Both roots are one tree here; `data/` is committed because a scheduled run starts from a fresh clone |
| 0062 | [Everything the archive needs is in the repository](0062-everything-the-archive-needs-is-in-the-repository.md) | docs | Knowledge that lives only outside the repository is knowledge the next session does not have |
| 0063 | [A kind that is accepted is offered](0063-a-kind-that-is-accepted-is-offered.md) | fix | The validator took four kinds and the reader was shown three, and a stored definition freezes the kind |
| 0064 | [A field with no validator is a field that goes missing](0064-a-field-with-no-validator-is-a-field-that-goes-missing.md) | fix | `models` was accepted, applied and never checked, so nine readings lost it without a word |
| 0065 | [Taking the update, and giving up our numbers](0065-taking-the-update-and-giving-up-our-numbers.md) | chore | Two sessions numbered from the same stale index; the one that had not pushed is the one that moves |
| 0066 | [One entity, many names](0066-one-entity-many-names.md) | feature | AIME 2024 was three records holding 9, 28 and 2 sources, each defined against its own fraction |
| 0067 | [A condition that its own effect falsifies](0067-a-condition-that-its-own-effect-falsifies.md) | feature | The write condition re-tested what the line above had just emptied, so five records were cleared and never written |
| 0068 | [Twenty-seven names ruled, and eleven refused](0068-twenty-seven-names-ruled-and-eleven-refused.md) | feature | Base and instruct are different weights; the largest ruling in the file is a refusal to merge |
| 0069 | [A counter that counts attempts](0069-a-counter-that-counts-attempts.md) | fix · superseded | Ours; `origin/main` reached the same defect independently and better in 0054 |
| 0070 | [The log line said seventy-six](0070-the-log-line-said-seventy-six.md) | fix · superseded | The result dict was already honest; the line a person reads during a run was not |
| 0071 | [The same collision, and the same bug](0071-the-same-collision-and-the-same-bug.md) | chore | Two sessions renumbered from one stale index again, and independently fixed one counter |
| 0072 | [A map of what the archive knows](0072-a-map-of-what-the-archive-knows.md) | docs | The wiki is organised by entity and states no order; this is the order, and what the archive does not hold |
| 0073 | [The cap bounds a run, not a decision](0073-the-cap-bounds-a-run-not-a-decision.md) | fix | A hand-filed PDF was archived, its document written, and no task filed — one WARNING among two dozen |
