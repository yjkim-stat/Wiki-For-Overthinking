...
| 0055 | [A model is not a dataset](0055-a-model-is-not-a-dataset.md) | feature · breaking | A checkpoint and a corpus answer different questions; a schema change has no seam to hide behind |
| 0056 | [Local extensions in a package of their own](0056-local-extensions-in-a-package-of-their-own.md) | feature | A placeholder is a false entity, a record with no abstract cannot be scored, and one cap starved the wiki |
| 0057 | [Three fixes that only made collection quieter](0057-three-fixes-that-only-made-collection-quieter.md) | fix | Under-collecting looks exactly like a quiet day, and a test inside a replaceable file cannot guard it |
| 0058 | [The archive this repository keeps](0058-the-archive-this-repository-keeps.md) | feature · breaking | Both roots are one tree here; `data/` is committed because a scheduled run starts from a fresh clone |
| 0059 | [Everything the archive needs is in the repository](0059-everything-the-archive-needs-is-in-the-repository.md) | docs | Knowledge that lives only outside the repository is knowledge the next session does not have |
| 0060 | [A kind that is accepted is offered](0060-a-kind-that-is-accepted-is-offered.md) | fix | The validator took four kinds and the reader was shown three, and a stored definition freezes the kind |
| 0061 | [A field with no validator is a field that goes missing](0061-a-field-with-no-validator-is-a-field-that-goes-missing.md) | fix | `models` was accepted, applied and never checked, so nine readings lost it without a word |
| 0062 | [Taking the update, and giving up our numbers](0062-taking-the-update-and-giving-up-our-numbers.md) | chore | Two sessions numbered from the same stale index; the one that had not pushed is the one that moves |
| 0063 | [One entity, many names](0063-one-entity-many-names.md) | feature | AIME 2024 was three records holding 9, 28 and 2 sources, each defined against its own fraction |
| 0064 | [A condition that its own effect falsifies](0064-a-condition-that-its-own-effect-falsifies.md) | feature | The write condition re-tested what the line above had just emptied, so five records were cleared and never written |
| 0065 | [Twenty-seven names ruled, and eleven refused](0065-twenty-seven-names-ruled-and-eleven-refused.md) | feature | Base and instruct are different weights; the largest ruling in the file is a refusal to merge |
...
| 0070 | [The cap bounds a run, not a decision](0070-the-cap-bounds-a-run-not-a-decision.md) | fix | A hand-filed PDF was archived, its document written, and no task filed — one WARNING among two dozen |
