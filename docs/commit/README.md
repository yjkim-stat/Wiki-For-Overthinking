...
**The notes numbered below 0103 predate this repository's git history.** They
were written against commits in an earlier tree that also held a working
archive, and that tree is not this one: the reasoning is what was carried over,
not the commits. A number is still fixed once pushed, and the sequence
continues from where it left off rather than restarting — a note that renumbered
itself would break every cross-reference the others make by number. Gaps in the
sequence are notes that described the archive's own residence rather than the
program, and were left behind with it.
...
| 0067 | [A note says when its own prose is behind](0067-a-note-says-when-its-own-prose-is-behind.md) | feature | Staleness was reported to whoever ran the render, not to whoever reads the note weeks later |
