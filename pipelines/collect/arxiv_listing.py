...
class Entry:
    ...
    abstract: str = ""
    # the announcement day this entry was listed under, ISO, or "" when
...
def _to_paper(entry: Entry) -> Paper:
    # the announcement day is the only date a listing page offers, and it
...
def collect(
    ...
    for category in categories:
        for page_index in range(max_pages):
            ...
            # `parse_days` rather than `parse_listing` — the day heading
...
def sweep(
    ...
    # record before the backfill, not only after it. The listing pass
