...
_KINDS = {
    ...
    # the `model` kind. The palette validates three entity hues and no
    ...
    # templates/wiki/graph.html would retire this.
...
def _shape(node: dict, x: float, y: float, r: float, css: str) -> str:
    ...
    if kind == "model":  # shares the concept hue, so the shape carries it
        d = r * 1.12
        pts = (
...
def _legend(present: set[str], edges: set[str] | None = None) -> str:
    order = ["topic", "concept", "method", "dataset", "model"]  # model
    marks = {
        ...
        "dataset": '<svg width="13" height="13"><polygon points="6.5,1 12,6.5 6.5,12 1,6.5" '
                   'class="mark k3"/></svg>',
        # same hue as concept, distinguished by the triangle.
...
def _table(nodes: list[dict]) -> str:
    # `model` shares the concept swatch; the Kind column is what separates
