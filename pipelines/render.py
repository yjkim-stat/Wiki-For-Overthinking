...
def _queue_and_summarizer(cfg: Config, max_pending=_UNSET):
    # `max_pending` is overridable so summaries can be held to a share of
...
def queue_missing_summaries(
    ...
    # a hand-filed PDF is not subject to the cap -- see
    ...
    for paper in store.iter_papers():
        ...
        if paper.is_local:
            ...
            continue
        # hand-filed papers go to the uncapped queue.
    ...
    # both queues' writes are reported together; they are one backlog.
    counts = {
        "unread": unread,
    ...
    *,
    ...
    if only in (None, "archive"):
        ...
        if not skip_queueing:
            # hold back part of the cap so definition tasks are not
    ...
    if only in (None, "wiki"):
        ...
        if not skip_queueing:
            ...
            # give any unused reserve back to the reading queue. Only
