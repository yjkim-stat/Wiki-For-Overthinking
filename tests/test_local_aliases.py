...
In a file of its own because the feature is: its call sites are one line each
in `common/config.py` and `enrich/concepts.py`, and anything that replaces
either of those wholesale would revert it without failing a test that lives
alongside the general code.
