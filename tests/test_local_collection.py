...
**Ours, not the template's.** These live in a file of their own for the reason
`docs/LOCAL-DELTAS.md` gives: a template update replaces
`tests/test_arxiv_listing.py` wholesale, and the fixes below were once lost
exactly that way — they were made in template-owned files, never registered as
deltas, and reverted the moment a newer template was adopted. A test in a file
the template does not own is what makes that loud instead of silent.
...
from a quiet day. `docs/LOCAL-DELTAS.md` entry 3 has the full account.
"""
