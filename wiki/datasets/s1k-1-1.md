...
s1K-1.1 is a small, curated reasoning dataset used across sources as a source of training data and as a comparison point for reasoning-compression and distillation methods, alongside similarly curated sets like LIMO. SelfBudgeter draws the model's own correct responses from s1K-1.1 (together with GSM8K and MATH) for its cold-start SFT phase that teaches a reasoning model to emit its own predicted token budget. CoRD treats s1K-1.1 (and LIMO-v1/v2) as a curated public baseline: reprocessing the same underlying question sets with CoRD's step-wise collaborative-teacher decoding at matched data scale consistently outperforms training on s1K-1.1 directly. A*-Thought and SuCo name it only as a reference dataset in their reasoning-compression work, without describing how it was built or used.
