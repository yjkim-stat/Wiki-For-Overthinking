...
A 32B Qwen trained with a group-relative variant, and across 3 sources the archive's reference reinforcement-learning-trained model in the reasoning-boundary debate. It is the model on which chain-scored pass@k reverses the plain-pass@k verdict, showing a consistent and significant gap in its favour against its base at every budget up to 1024. It is also where symbolic variabilisation shows scaling raising raw accuracy without fixing consistency -- 92.3 to 69.8 percent on one set under the strict metric, with the loose partial-credit metric narrowing that to 7.2 percent, which separates memorisation from inconsistency. And it is one of the four model families in the branch-preference measurement showing entropy collapse concentrated in semantic rather than syntactic contrasts.

- **Kind**: model
- **Also called**: DAPO-Qwen-32B, DAPO-Qwen2.5-32B
