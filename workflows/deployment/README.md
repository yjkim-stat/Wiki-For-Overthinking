...
cp ~/ra-wm/config/settings.yaml ~/ra-wm/config/sources.yaml config/
cp ~/ra-wm/.gitignore .gitignore          # keeps PDFs, logs and raw responses out
```
...
```bash
cd ~/ra-wm && scripts/new_topic.sh "Causal Inference"    # writes into this checkout
...
```bash
git -C ~/ra-wm rev-parse HEAD > ~/research-archive/FRAMEWORK.txt
```
...
```bash
git -C ~/ra-wm rev-parse HEAD > FRAMEWORK.txt
